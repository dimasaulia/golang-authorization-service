...
func (m *AuthModuleImpl) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /api/v1/auth/keycloak/redirect", m.AuthController.KeycloakRedirect)
	mux.HandleFunc("GET /api/v1/auth/keycloak/callback", m.AuthController.KeycloakCallback)
	mux.HandleFunc("POST /api/v1/auth/keycloak/exchange", m.AuthController.KeycloakExchange)
