...
func NewActionRepository(db *database.Database, appLogger *logger.Logger) ActionRepository {
	return &ActionRepositoryImpl{
		db:  db,
		log: appLogger.Layer("repository.actions"),
...
func (r *ActionRepositoryImpl) Find(ctx context.Context, params shared.ListParams) ([]entities.Action, error) {
	...
	return pgx.CollectRows(rows, pgx.RowToStructByName[entities.Action])
}

func (r *ActionRepositoryImpl) FindByID(ctx context.Context, id int64) (*entities.Action, error) {
...
func (r *ActionRepositoryImpl) Create(ctx context.Context, entity entities.Action) (*entities.Action, error) {
	...
	created, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[entities.Action])
	if err != nil {
		return nil, err
	}
...
func (r *ActionRepositoryImpl) Update(ctx context.Context, id int64, data map[string]any) (*entities.Action, error) {
	if len(data) == 0 {
	...
	updated, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[entities.Action])
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, ErrNotFound
	}
	if err != nil {
...
func (r *ActionRepositoryImpl) Delete(ctx context.Context, id int64) error {
