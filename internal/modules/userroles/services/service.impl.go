...
func (s *UserRoleServiceImpl) Find(ctx context.Context, limit uint64, offset uint64) ([]entities.UserRole, error) {
	end := s.log.Start(ctx, "Find")
	items, err := s.UserRoleRepository.Find(ctx, limit, offset)
