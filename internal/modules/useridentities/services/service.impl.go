...
func (s *UserIdentityServiceImpl) Find(ctx context.Context, limit uint64, offset uint64) ([]entities.UserIdentity, error) {
	end := s.log.Start(ctx, "Find")
	items, err := s.UserIdentityRepository.Find(ctx, limit, offset)
