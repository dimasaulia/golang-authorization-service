...
func Initialize(ctx context.Context) (*App, error) {
	...
	authRoleRepository := repositories11.NewRoleRepository(databaseDatabase, logger)
	authUserRoleRepository := repositories13.NewUserRoleRepository(databaseDatabase, logger)
	authService := services22.NewAuthService(configConfig, redisRedis, userRepository, accessRepository, authRoleRepository, authUserRoleRepository, keycloakClient, freeIPAClient, logger)
	...
	authModuleImpl := auth.NewAuthModule(authController, configConfig, authenticator)
	...
	moduleModuleImpl := modules.NewModuleModule(moduleController, configConfig, authenticator)
	...
	actionModuleImpl := actions.NewActionModule(actionController, configConfig, authenticator)
	...
	permissionModuleImpl := permissions.NewPermissionModule(permissionController, configConfig, authenticator)
	...
	menuModuleImpl := menus.NewMenuModule(menuController, configConfig, authenticator)
	...
	teamModuleImpl := teams.NewTeamModule(teamController, configConfig, authenticator)
	...
	teamRoleModuleImpl := teamroles.NewTeamRoleModule(teamRoleController, configConfig, authenticator)
